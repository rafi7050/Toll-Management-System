...
class Package(TimeStamp, OperatorStamp):
    ...
    size = models.IntegerField(choices=SIZE, default=1, null=True, blank=True)
    age_group = models.ForeignKey(AgeGroup, on_delete=models.SET_NULL, null=True, blank=True)
    nutrition_point = models.ForeignKey(NutritionPoint, on_delete=models.SET_NULL, null=True, blank=True,
                                        verbose_name='Cure Disease Package')
    is_cure_disease_package = models.BooleanField(default=False)
