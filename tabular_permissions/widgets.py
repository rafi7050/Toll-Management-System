...
class TabularPermissionsWidget(FilteredSelectMultiple):
    ...
    def __init__(self, verbose_name, is_stacked, permission=Permission.objects.all().select_related('content_type'),
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        all_perms = self.permission.values('id', 'codename', 'content_type_id').order_by('codename')
        excluded_perms = set([])
        codename_id_map = {}
        for p in all_perms:
        ...
        response = ''.join([force_text(body), force_text(initial)])
